    ...
    for (lower,upper,index) in decision_boundary:
        ...
        #********       MODE ONE        ********
            ...
            if(lower<=zreading<upper):
                if(off==1):
                    msg=Message("note_off", channel=0,note=play_note, velocity=100)
                    outport.send(msg)       #if mode switched from pan, switch off pan note
                if(previousnote!=int(current_note)):                 #if note is different from last, switch it off
                    on=1
                    msg=Message("note_off", channel=0,note=previousnote, velocity=100)
                    outport.send(msg)
                if(on==1):
        ...
        #********       MODE TWO        ********
            ...
            if(lower<=zreading<upper):
                ...
                if(off==0):
                        ...
                        msg=Message("note_on", channel=0,note=int(current_note), velocity=100)
                        outport.send(msg)      #This will be switched off only if mode changes
                time.sleep(0.3)
                off=1
                cc = Message("control_change", channel= 0, control= 10 , value=xreading) 
                outport.send(cc)                      #CC: Pan
                time.sleep(0.1)
                break
        ...
        #********       MODE THREE     ********
            ...
            if(lower<=zreading<upper):
                ...
                time.sleep(0.3)
                off=1
                cc = Message("control_change", channel= 0, control= 12 , value=xreading) 
                outport.send(cc)                     #CC: noise 
                time.sleep(0.1)
                break
