    ...
    for (lower,upper,index) in decision_boundary:
        if(count%2==0):
            if(lower<=zreading<upper):
                if(off==1):
                    midi_out.send_message([0x80,previousnote1,100])
                if(previousnote2!=int(notes[index])):
                    on=1
                    midi_out.send_message([0x80,previousnote2,100])
                if(on==1):
                    print "Playing note- " + str((notes[index]))                #display which note is playing
                    previousnote2=int(notes[index])
            ...
            if(lower<=zreading<upper):
                ...
                if(off==0):
                       previousnote1=int(notes[index])
                       midi_out.send_message([0x90, int(notes[index]),100])
                time.sleep(0.3)
                off=1
